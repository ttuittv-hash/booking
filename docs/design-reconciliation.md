...
| `text-r` / `text-l` | **각 1곳** | 팔레트에서 빼도 비용이 사실상 없다 |
...
| G9 | 루트 글자 크기를 화면 폭에 매달기 | **DEFER** | token | 효과(줄바꿈 유지)는 실재하지만 `html` 글자 크기를 건드리는 일이라 브라우저 확대·최소 글자 크기와 상호작용한다. 사이트 전체에 걸리므로 **별도 승인 후** |
| G10 | `.header-scale-lock` | **DEFER** | token | G9 에 종속 — G9 없이는 존재 이유가 없다 |
| G11 | `text-r`·`text-l` 팔레트에서 빼기 | **RESTORE** | token | 각 **1곳**만 쓰인다. 제거 비용이 사실상 없고 본문 단이 셋으로 줄어 일관성이 오른다 |
...
**KEEP CURRENT 8** · **RESTORE 8**(부분 2 포함) · **REIMPLEMENT 10** · **DROP 5** ·
**DEFER 2** · **보류 1**
...
- **G9 · G10** — 루트 글자 크기를 화면 폭에 매다는 방식. 1024 이상에서만 작동해 모바일에는
  영향이 없고, 중간 폭에서 제목 줄바꿈이 유지되는 실질 이점이 있다. 다만 `html` 글자
  크기를 건드리므로 브라우저 확대·사용자 기본 글자 크기 설정과 상호작용한다.
  **넣을지 말지 지시를 받고 진행한다.**
