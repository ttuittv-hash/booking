...
| G6 | 입력 포커스 표시 | **적용 완료 → 요청으로 재조정** | token | ① 옐로 아웃라인을 검정으로 바꿨다(1.5:1 → 21:1, WCAG 3:1 미달 해소). **문서가 「옐로는 포커스 링과 밑줄 강조에만」이라 규정하고 있어 그 줄도 고쳤다** — 같은 문서가 「옐로 글자는 1.5:1 로 읽히지 않는다」고 인정하던 내부 모순이다. ② 이어 요청으로 **링을 없애고 면으로 바꿨다** — `outline: none` + 초점 시 배경이 오프화이트(투명) → **흰 면**(`--panel`). 초점 표시는 테두리(#CCC → 검정, 21:1)가 맡고 면 변화는 보조 신호다(1.05:1) |
...
| 항목 | 전 | 후 | 근거 |
|---|---|---|---|
| 입력 포커스 (G6) | 옐로 아웃라인 2px | **검정 테두리 + 흰 면, 링 없음** | 옐로는 오프화이트 지면에서 **1.5:1** — 초점이 어디인지 보이지 않는다. 검정 테두리는 21:1 로 WCAG 3:1 을 넘는다. 링은 요청으로 뺐고, 면이 흰색으로 떠오르는 것이 「칸이 열렸다」는 신호를 맡는다 |
...
- **모든 입력칸의 초점 표시**가 옐로 링 → **검정 테두리 + 흰 면**(링 없음).
  가장 넓게 퍼지는 변화다
- **백오피스 체크박스 7곳**이 옐로 → 검정
- 설정이 없던 체크박스들이 **브라우저 기본색(사파리 파랑) → 검정**
- `select` 에 셰브런이 생기고 오른쪽 여백이 잡힌다

### 시각 검증 (2026-09-09) — 실제 렌더링으로 확인

로컬에 `embedded-postgres`(5433) + `next dev` 를 띄우고, 폼 컨트롤을 한 화면에 모은
임시 페이지로 1000px 폭에서 확인했다. **계산값과 렌더링이 일치한다.**

| 자리 | 계산값 | 눈으로 본 것 |
|---|---|---|
| 밝은 지면 체크박스 | `accent-color: rgb(0,0,0)` | 검정 채움 · **흰 체크** |
| 검정 상자 안 (`bg-inverse-bg`) | `accent-color: rgb(255,255,255)` | 흰 채움 · **검정 체크** (라디오도 흰 채움·검정 점) |
| 선택된 칩 안 (`CHOICE_SELECTED_VARS`) | 같음 | 흰 채움 · 검정 체크 |
| 입력칸 초점 | `background: rgb(255,255,255)` · `border: rgb(0,0,0)` · `outline-style: none` | 흰 면으로 떠오르고 테두리가 검정, 링 없음 |
| `select` | — | 셰브런이 오른쪽 여백 안에 놓인다 |

체크 표시(✓)의 색은 우리가 지정하지 않는다 — 브라우저가 채움색 대비에 맞춰 정하고,
검정 채움에는 흰 체크, 흰 채움에는 검정 체크가 나오는 것을 확인했다.

**환경 메모** — `playwright` 를 새로 설치하면 요구 브라우저 리비전이 캐시에 있는 것과
어긋난다(`chromium_headless_shell-1243` 요구 vs 캐시 `chromium-1234`). 내려받지 말고
`executablePath` 로 캐시의 `chromium-1234/chrome-mac-arm64/Google Chrome for Testing.app/
Contents/MacOS/Google Chrome for Testing` 를 가리키면 된다. 스크립트는 **저장소 안**에
두어야 모듈을 찾는다.
