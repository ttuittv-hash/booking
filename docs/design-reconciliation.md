...
| G9 | 루트 글자 크기를 화면 폭에 매달기 | **적용 완료** (승인 2026-09-09) | token | `html { font-size: min(1rem, 100vw / 90) }`, `lg` 이상. 컴파일 확인 `html{font-size:min(1rem,1.11111vw)}`. 브라우저 글자 크기 설정이 이 구간에서 가려지는 트레이드오프는 `globals.css` 주석과 문서 §반응형에 명시했다 |
