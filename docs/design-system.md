...
| `md` | 40 | 폼 제출 · 탭·칩(이용 시설, 위저드 하위 단계). 백오피스·인증 화면 |
| `sm` | 32 | 카드·표 안의 인라인 액션 · 토글(`toggleClass`) · 아이콘 버튼(`ICON_BTN_SM`) · 삭제 |

**같은 줄에 놓이는 컨트롤은 전부 같은 높이로 맞춘다** — 한 줄에 40과 48이 섞이면 줄이 흔들린다.
임의 픽셀 조합(`px-3.5 py-2`)으로 새 크기를 만들지 않는다. 인라인 토글은 `toggleClass(selected)`,
±·‹› 같은 아이콘 버튼은 `ICON_BTN_SM` 을 쓴다.
