...
- **초점은 고대비 한 겹이다** — 테두리를 검정으로 바꾸고 같은 색 아웃라인 2px 을 얹는다.
  옐로 아웃라인을 쓰던 자리인데 오프화이트 지면에서 **1.5:1** 이라 초점 표시가 사실상
  보이지 않았다. 키보드로 도는 사람은 지금 어디에 있는지를 잃는다. 검정은 21:1 이다.
...
정한다: 지면 위에서는 검정 채움, 선택된 칩(검정 면) 안에서는 흰 채움
(`CHOICE_SELECTED_VARS` 가 넘기는 `--check-fill`), `bg-inverse-bg` 안에서도 흰 채움.
