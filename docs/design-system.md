...
출처
- **디자인 가이드 v0.1 (2026-08-20)** — 경험기획실 디자인팀
  (`iCloud › SeoulArena/경험기획실/디자인팀/웹사이트/260820 서울아레나 대관시스템 디자인·브랜드 가이드.md`)
- Kakao Arena Brand Guidelines 0.1
- Figma `2608 서울아레나 대관시스템` (`00 Foundations` / 공개·신청자·운영자 화면)
- Notion `(웹사이트) 대관 사이트 8/20 오픈 기준 정보구조 재구성 및 페이지별 콘텐츠 전문`
...
### 페이지 그리드 — 6컬럼 (디자인 가이드 §2)

**6 columns / Stretch / Margin 64 / Gutter 40.** 모든 요소는 컬럼 경계에 스냅한다.
...
뷰포트 1497 기준 → 콘텐츠 1369 · 1col 194.8 · **2col 429.7** · **3col 664.5** · **4col 899.3**

| 유틸리티 | 쓰는 자리 |
|---|---|
| `container-site` | 섹션 안쪽 래퍼. max-width 없이 좌우 마진만 준다 |
| `grid-site` | 6컬럼 그리드. 자식은 `lg:col-span-*` 로 스팬을 적는다 (1024 미만 1컬럼) |
| `measure` | 산문 문단 최대 폭 768. 컬럼 스팬과 무관한 절대값 |

브레이크포인트별 마진/거터: `≥1440` 64/40 · `1024` 48/32 · `768` 32/24 · `<768` 20/16

**구 규격 폐기** — `max-width 1384 + padding 64`(콘텐츠 1256) 체계는 더 쓰지 않는다.
코드에서 **1256 · 458 · 522 · 734** 를 보면 그것은 구 규격 잔재다.

허용되는 다단은 넷뿐이다 — 설명+데이터(2col+4col) · 본문+사이드바(4col+2col) ·
2-up 카드(3col×2) · 3-up 요약 카드(2col×3). 그 밖에는 한 컬럼으로 흐른다.

### 절차·순서는 가로 한 줄로 (§8)

**순서가 있는 내용을 3열 카드 그리드로 배치하지 않는다.** 읽는 순서가 `1→2→3 / 4→5` 로
꺾여 선형성이라는 정보의 본질이 깨진다. `ProcessDiagram`(가로 칩 + 셰브런) +
`ProcessDetail`(하단 상세)을 쓴다. 칩 폭 = `(콘텐츠 폭 + 24) / n − 24`, 단계는 최대 8개.

### 표 (§6)

표는 자체 그리드를 만들지 않고 **놓인 컨테이너의 그리드 컬럼에 스냅**한다.

| 표 열 수 | 모두 텍스트 | 마지막이 금액 |
|---|---|---|
| 2열 | `[1] [2]` | `[1] [last]` |
| 3열 | `[1] [2] [3]` | `[1] [2] [last]` |
| 4열 | `[1] [2] [3] [last]` | 동일 |

라벨·텍스트는 좌측, **숫자와 금액은 우측 정렬**이며 금액 열은 마지막 컬럼으로 민다.
값 열이 여러 개인 비교표(패키지 1~4)에서는 금액도 각 열에 두고 마지막 열로 밀지 않는다.
행 높이 헤더 49 / 본문 42, 하단 구분선만(헤더 `rgba(0,0,0,.25)` · 본문 `.15`),
세로 구분선·줄무늬 없음.
...
| `SiteFooter` | Figma Design › Footer / 1. 상단 Address·Contact + 사이트맵 → **컨테이너 전폭 워드마크** → 헤어라인 → 카피라이트·정책. 오프화이트 지면. 대외 문의 이메일은 창구 확정 전까지 노출하지 않고 1:1 문의로 연결한다 |
| `ProcessDiagram` / `ProcessDetail` | 절차·순서의 유일한 표현. 3열 카드 그리드 금지 |
| `NumberedNotes` | 번호가 붙은 유의사항 목록 |
| `StatBand` | KEY NUMBERS 스탯 밴드 |
| `DownloadCard` / `DownloadIcon` | 자료실 카드. 액션 아이콘은 24 박스 / 16 글리프 |
| `ReleaseNotice` | 미공개 페이지의 대체 화면. 라우트를 죽이지 않고 "9/1 공개"를 알린다 |
| `QueryTabs` | URL 쿼리를 정본으로 삼는 탭. 인쇄 시 모든 패널을 출력한다 |
...
## 7. 콘텐츠 정본의 소유 (2026-08 정보구조 재구성)

같은 값을 두 곳에서 편집할 수 있으면 반드시 어긋난다. 수치 불일치는 대관 분쟁의 씨앗이다.
아래 표 밖의 곳에서 같은 값을 다시 적지 않는다.

| 값 | 정본 | 비고 |
|---|---|---|
| 시설 제원·부속 공간·무대 특장 수치 | `src/lib/content/venueFacts.ts` | 시설소개자료 PDF V1.0 (2026-08-18) |
| 대관 절차 8단계 · 신청 전 확인 사항 | `src/lib/content/processFacts.ts` | 홈은 3구간으로 압축해 참조 |
| 요금 체계·금액 | `src/lib/content/rateFacts.ts` + 요금표 관리 화면 | 대관 오픈 준비 요금 시트 |
| FAQ 20문항 (교통·주차 정본) | DB `faqs` (`src/lib/content/faqSeed.ts` 로 최초 1회 시드) | 운영자 편집 대상 |
| **대관 일정** | DB `notices` 의 `대관공고` 게시물 | 홈·대관 안내의 공고 카드는 이 값을 끌어다 쓴다 |
| 홈 히어로·브랜드 선언문 카피 | DB (`/admin/content` › 홈 화면) | 실제로 자주 바뀌는 카피만 CMS 에 남겼다 |

**근거가 없는 수치는 쓰지 않는다.** 잔향 2.8초 · C Value 208.94 · 무대~객석 후미 90m ·
제작비 30% 절감 · 관계자 주차 200대 · 컨벤션홀 1,400명은 근거 문서가 없어 전면 배제했다.
확정 전에는 "추후 공지" 또는 "확정 후 안내"로 두고 다른 숫자로 대체하지 않는다.

## 8. 공개 시점 게이트

`src/lib/release.ts` — 8/20 대관공고 / 9/1 대관오픈 두 단계.
미공개 페이지는 라우트를 살리고 `ReleaseNotice` 로 대체하며 **메뉴에서 숨기지 않는다.**
북마크·검색 유입·공유 링크가 죽지 않게 하고, 방문자에게 무엇이 언제 열리는지 알리는
가장 싼 수단이기 때문이다. 미리보기는 `SEOUL_ARENA_RELEASE_PHASE=open|notice`.

9/1 공개 대상 — 시설 제원 · 부대시설 · 커넥티드 라이브 · 내 신청 내역 · 신청 위저드.
