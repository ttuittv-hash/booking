...
- 사이드바 — **처음 화면부터 「실시간 견적 요약」**이다(항목 · 소계 · 부가세 · 합계).
  **박스는 검정 1px 테두리**다 — 대관료를 옐로 2px, 추후 정산을 회색 2px 로 갈랐던 동안
  한쪽이 더 중요한 것처럼 보였다. 굵기·색을 하나로 맞추고, 둘을 가르는 일은 안쪽 합계
  줄(옐로=계약금액 / 검정=추후 정산)에 맡긴다. 면은 지면과 같은 오프화이트다. 「계약 기준 금액」·「변동 가능」 배지도 **면 색만** 쓴다(10px 글자에
