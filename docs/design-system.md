...
| `PublicHeader` | 높이 `--header-h`(56 / lg 64). **아웃라인 없음** — 지면과 같은 색이고 스크롤하면 `bg-background/85 + blur` 로 바뀌어 글자 가독성만 지킨다. **넓은 화면(lg 이상)** — 좌 워드마크 / 중앙 `YOUR STAGE  YOUR GUIDE  [BOOK IT]` (올캡스 Archivo 14, **구분선 없음**, 좌우 요소 사이가 아니라 **화면 중앙에 절대 위치** — 아래 알약 탭과 같은 축. `BOOK IT` 만 채움 버튼 h32) / 우 지원▾ · 이름▾ · 로그아웃 · 알림(**맨 오른쪽**) (**12** — 중앙보다 커 보이지 않게 한 단 작게, 같은 14 라도 국문이 더 무겁다). **카테고리는 호버해도 색이 변하지 않는다** — 갈 페이지가 없으므로 누를 수 있다는 신호를 주면 안 된다. 펼쳐지는 패널이 피드백이다. **카테고리가 아닌 항목(BOOK IT·워드마크·우측 유틸)에 마우스가 닿으면 드롭다운을 즉시 닫는다** — 유예(120ms)만 믿으면 지나가는 동안 패널이 버튼을 덮어 오클릭이 난다. 우측 유틸은 채움·아웃라인 없는 텍스트 버튼. **좁은 화면** — 원형 클릭 메뉴 유지(닫힘 = 검정 채움 원 / 열림 = 아웃라인 원 + 내부 ×, 지름이 같아 아이콘이 튀지 않는다) |
...
- 진행 표시 — **막대가 아니라 5개 그룹 탭**이다. 내부 스텝은 8개지만 순번이 인접하지 않은
  스텝을 한 그룹으로 묶으므로(04 기본 정보 = 5·6·7, 05 신청서 제출 = 4·8) 퍼센트 막대를
...
- **이전/다음 버튼 줄은 화면당 하나**다. 하단 우측에 이전(아웃라인) + 다음(검정 채움), 높이 48.
  상단에 옅은 알약 버튼을 따로 두지 않는다 — 같은 동작이 두 모양이면 어느 쪽이 진짜
  진행인지 헷갈린다
- 사이드바 — 진행 중에는 **금액을 감춘 「선택 내용」**만 보여 준다. 금액은 제출 직전
  「예상 대관료」에서 한 번만 확인시킨다 (입력 중 금액이 오르내리면 확정가로 오해한다)
...
- **구성·옵션에 색을 새로 들이지 않는다.** 기본 포함 = 아웃라인 박스 + 아웃라인 배지,
  선택 옵션 = **박스 자체가 검정 면** + 흰 채움 배지, 그 위에 항목은 **흰 카드**.
  초록·경고색 색면을 쓰지 않고, 탭 활성·체크박스도 검정이다. 옐로는 일정 캘린더에만 남긴다
- **입력칸이 있는 항목은 선택 여부로 색을 바꾸지 않는다.** 면 색이 바뀌면 입력한 숫자가
  묻힌다 — "고르는 곳"이라는 신호는 바깥 박스가 준다
- 검정 면은 `INVERSE_SURFACE_VARS` 로 토큰을 국소 반전하고, **그 안의 흰 카드는
  `PLAIN_SURFACE_VARS` 로 되돌린다.** 되돌리지 않으면 흰 카드 안 글자가 흰색으로 남는다
