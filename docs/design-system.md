...
#### 스팬은 1 · 2 · 3 · 4 · 6 만 쓴다 — 한 줄에 4개를 균등 분할하지 않는다

6 을 4 로 나눌 수 없어서(1.5칼럼) **4-up 균등 분할은 어떤 거터로도 그리드에 올라가지
않는다.** 12칼럼으로 쪼개도 마찬가지다 — 4-up 의 안쪽 경계는 반칼럼 자리에 떨어진다.
대등한 항목을 여러 개 놓을 때는 **2col 씩(3-up)** 또는 **3col 씩(2-up)** 이다.
항목이 4개면 3-up + 남는 하나로 두 줄이 되는 게 정상이다.

시설 소개 맨 위 개요(공연장 형태 · 공연 가능 형태 · 좌석 운영 · 무대 구성)가 4-up 이라
두 번째 카드가 아래 두 칼럼 섹션의 오른쪽 칼럼보다 왼쪽에서 시작했고, 한 화면에 축이
두 개로 보였다. 2col 씩으로 바꿔 두 번째 카드가 오른쪽 칼럼과 같은 선에서 시작한다.

#### 두 칼럼은 항상 2col + 4col

규약(목차+본문) · 마이페이지(메뉴+본문) · FAQ(묶음+질문) · `SplitSection`(제목+표) ·
위저드(본문+견적 요약)가 모두 같은 분할이다. 사이드바를 `360px` 처럼 고정 폭으로 두면
경계가 그리드와 어긋나 화면 안에 축이 두 개가 된다(위저드가 그랬다).

#### 그리드에 맞추지 않는 것 (의도된 예외)

폭이 **내용에서 나오는 것**은 칼럼에 맞추지 않는다 — 맞추려면 빈 공간을 넣어야 한다.

| 예외 | 이유 |
|---|---|
| 버튼·버튼 묶음 | 폭이 라벨에서 나온다. 우측 액션 묶음은 콘텐츠 오른쪽 끝에 붙인다 |
| 인라인 통계 쌍 (SEATED · STANDING, 버전·시행일·구성) | 값 옆에 값을 붙여 읽는 것이 목적 |
| 번호 + 문장 목록 (홈 선언문, 서울아레나 개요) | 번호는 글머리다. 1칼럼(182px)에 넣으면 번호와 문장이 멀어진다 |
| 대관 절차 8단계 (한 줄 4박스 + 화살표) | 칼럼이 아니라 **체인**이다. 정본(§8)이 4박스로 지정 |
| 표 내부의 라벨 열(12rem) | 표는 자기 트랙 안에서 다시 나뉜다 |
| 상단바·푸터 | 페이지 그리드가 아니라 크롬(중앙 절대 위치 · `1fr auto`) |
