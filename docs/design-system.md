...
- 옐로를 면·글자색으로 쓰지 않는다 — 옐로는 **밑줄 강조**에만
  (흰 지면 위 옐로 글자는 1.5:1 로 읽히지 않는다).
  **포커스 링에도 쓰지 않는다** — 같은 1.5:1 이라 초점 표시가 보이지 않고, WCAG 가
  포커스 표시에 요구하는 3:1 에 못 미친다. 초점은 검정(21:1)이다. 아래 「입력칸」 참고
  (2026-09-09 정정 — 그전에는 이 줄이 「포커스 링과 밑줄 강조에만」이라 적혀 있었다)
...
- **기본부터 옐로 2px 인 자리는 다르다**(`decoration-accent decoration-2`, 8곳) — 파일
  다운로드·전자 날인처럼 **브랜드 강조**가 목적이고 글자는 검정(`font-bold text-foreground`)
  이라 가독성에 영향이 없다. 밑줄은 장식이므로 옐로를 써도 된다.
...
- 사이드바 — **처음 화면부터 「실시간 견적 요약」**이다(항목 · 소계 · 부가세 · 합계).
  **박스는 테두리 없이 흰 면**이다 — 대관료를 옐로 2px, 추후 정산을 회색 2px 로 갈랐지만
  안쪽 합계 줄이 이미 옐로(계약금액)·검정(추후 정산)으로 구분을 맡고 있어 테두리가
  겹치는 신호였다. 「계약 기준 금액」·「변동 가능」 배지도 **면 색만** 쓴다(10px 글자에
