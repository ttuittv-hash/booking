...
**루트 글자 크기를 화면 폭에 매단다** — `html { font-size: min(1rem, 100vw / 90) }`,
`lg` 이상에서만.
...
- **이 구간에서는 브라우저 글자 크기 설정이 가려진다.** `min()` 이 둘 중 작은 값을
  고르므로 사용자가 기본 크기를 키워도 `100vw / 90` 이 더 작으면 그 값이 이긴다
  (1024 폭에서 11.4px). 축소가 1024~1440 으로 한정되고 그 아래에서는 설정이 살아
  있다는 것을 전제로 받아들인 트레이드오프다.
