...
- **구성·옵션에 색을 새로 들이지 않는다.** 기본 포함 = 아웃라인 박스 + 아웃라인 배지,
  선택 옵션 = **박스 자체가 검정 면** + 흰 채움 배지. 초록·경고색 색면을 쓰지 않고,
  탭 활성·체크박스도 검정이다. 옐로는 일정 캘린더에만 남긴다
- **수량 입력칸이 있는 행은 면을 채우지 않는다.** 채우면 입력한 숫자가 배경에 묻힌다 —
  "고르는 곳"이라는 신호는 바깥 박스가 주고 안쪽 항목은 색이 변하지 않는다.
  검정 면을 만들 때는 `INVERSE_SURFACE_VARS` 로 토큰을 국소 반전해 입력 필드·보조
  텍스트·헤어라인이 지면에 맞게 따라오도록 한다
