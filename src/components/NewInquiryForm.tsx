...
export function NewInquiryForm({
  ...
  const categories = useMemo(() => visibleInquiryCategories(rentalOpen), [rentalOpen]);
    ...
    <div className="border-t border-border/25 pt-6">
      <div className="space-y-6">
        ...
        {showQuote && (
          <label className="block">
            <span className="mb-2 block text-xs font-bold">
              관련 신청번호{!quoteRequired && <span className="ml-1 font-normal text-muted">(선택)</span>}
            </span>
            {myQuoteIds.length > 0 ? (
              <select
                value={quoteId}
        ...
        <div>
          ...
          <p className="text-s">{notifyEmail}</p>
          <span className="mt-2 block text-xs text-muted">
