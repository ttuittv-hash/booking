...
// [화면 뼈대 2026-08-20, 재개정] 진행 표시를 다시 5개 그룹으로 나눈다 — 01 공간 선택 /
// 02 일정 선택 / 03 구성·옵션 / 04 기본 정보(신청자 정보 → 관객 → 공공성) / 05 신청서
// 제출(예상 대관료 → 최종 제출). 이용 시설(공간) 선택과 일정(캘린더) 선택을 별개
// 슬롯으로 분리했다 — 공간을 먼저 고르면 그 다음 슬롯(일정 선택)의 내용(아레나 캘린더 /
...
// 8(최종 제출)만 담아서 순번상 인접하지 않은 스텝을 한 그룹으로 묶는다.
interface SubStep {
  step: number;
  label: string;
}
...
const STAGE_GROUPS: StageGroup[] = [
  { label: "01 공간 선택", steps: [{ step: 1, label: "공간 선택" }] },
