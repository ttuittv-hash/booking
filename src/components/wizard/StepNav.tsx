...
// [화면 뼈대 2026-08-19, 개정] 진행 표시는 4개 그룹으로 묶는다 — 01 패키지 선택 /
// 02 구성·옵션 / 03 기본 정보(신청자 정보 → 관객 → 공공성) / 04 신청서 제출(예상 대관료 →
// 최종 제출). "패키지 선택"은 그룹 이름일 뿐, 실제 화면(STEP 1-1)은 시설·무대구성·규모를
// 입력하면 패키지가 자동으로 결정되는 구조다 — Package 1~4를 카드로 골라 클릭하는 화면은
// 없다(4개 패키지는 Bowl 사용료만 차등일 뿐 구성이 완전히 동일하다는 확정 사항 때문).
// 위저드 내부 스텝 번호(1~8)는 그대로 두고 이 그룹에 여러 개씩 묶는다 — 03 기본 정보는
// 5(신청자 정보)·6(관객)·7(공공성)만, 04 신청서 제출은 4(예상 대관료)·8(최종 제출)만 담아서
// 순번상 인접하지 않은 스텝을 한 그룹으로 묶는다(심사·결과 안내 그룹은 제거, 2026-08-19;
// 객석배치도/공공성 계획서 첨부 슬롯을 분리하며 관객·공공성도 별개 스텝으로 분리, 2026-08-19).
interface SubStep {
  step: number;
  label: string;
}
