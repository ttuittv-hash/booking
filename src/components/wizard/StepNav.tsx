...
// 02 구성·옵션 / 03 신청서 제출(예상 대관료 → 최종 제출) / 04 기본 정보(신청자 정보 →
// 관객·공공성). "패키지 선택"은 그룹 이름일 뿐, 실제 화면(STEP 1-1)은 시설·무대구성·규모를
// 입력하면 패키지가 자동으로 결정되는 구조다 — Package 1~4를 카드로 골라 클릭하는 화면은
// 없다(4개 패키지는 Bowl 사용료만 차등일 뿐 구성이 완전히 동일하다는 확정 사항 때문).
// 위저드 내부 스텝 번호(1~7)는 그대로 두고 이 그룹에 여러 개씩 묶는다 — 03 신청서 제출은
// 4(예상 대관료)·7(최종 제출)만, 04 기본 정보는 5(신청자 정보)·6(관객·공공성)만 담아서
