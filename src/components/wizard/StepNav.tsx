...
// "01 일정 선택"이다. 이용 시설(공간)을 먼저 고르면 그 아래 일정 슬롯(아레나 캘린더 /
// 중형 캘린더 / 동시 대관 탭)이 그 선택에 따라 달라진다. 관객 규모는 여전히 구성·옵션에서
// 입력한다. 위저드 내부 스텝 번호(1~7)는 그대로 두고 이 그룹에 여러 개씩 묶는다 —
// 03 기본 정보는 4(신청자 정보)·5(관객)·6(공공성)만, 04 신청서 제출은 3(예상 대관료)·
// 7(최종 제출)만 담아서 순번상 인접하지 않은 스텝을 한 그룹으로 묶는다.
interface SubStep {
  step: number;
  label: string;
}
