...
export function Step6Submit({
  ...
  return (
    <section className="rounded-lg border border-border bg-background p-7">
      ...
      <div className="mt-6 flex flex-wrap items-center justify-between gap-4 rounded-lg border border-border bg-panel/60 p-6">
        <div>
      ...
      {submittedId ? (
        <div className="mt-5 rounded-md border border-good/30 bg-good-soft px-4 py-3.5 text-[13.5px] text-good">
          신청서가 생성되었습니다. 신청번호{" "}
        ...
        <div className="mt-5 rounded-md border border-warn/30 bg-warn-soft px-4 py-3.5 text-[13.5px] text-warn">
        ...
        <>
          <button
            ...
            disabled={submitting}
            onClick={onSubmit}
            className="mt-5 rounded-md bg-accent px-7 py-3 text-[14px] font-semibold text-white transition-colors hover:bg-accent-hover disabled:opacity-50"
          >
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
        {STAGES.map((s) => (
          <div key={s.no} className="rounded-md border border-border bg-panel/60 p-4">
