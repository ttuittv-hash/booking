    ...
    <div className="rounded border border-border bg-panel/60 p-6">
      ...
      {deposit.status === "PENDING" && viewerRole === "APPLICANT" && (
        <div className="mt-4 space-y-3">
          <p className="rounded border border-warn/30 bg-warn-soft px-3 py-2.5 text-[12.5px] text-warn">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 rounded border border-border bg-background px-3 py-2 text-[13px] outline-none focus:border-accent"
            />
            <button
              type="button"
...
function StatusBadge({ status }: { status: Deposit["status"] }) {
  ...
  return (
    <span className={`rounded-sm px-2.5 py-1 text-[11.5px] font-medium ${style}`}>{label}</span>
  );
}
