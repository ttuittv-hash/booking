    ...
    <ContentFormShell page="screenText" initial={content}>
      {(v, patch) => (
        <>
          ...
          {/* [신규 2026-09-06] "대관 위저드에서 각 슬롯 내 항목들 순서 조정 + 노출 On/off"
              — 신청자 정보/공연 정보(STEP3 대관정보 슬롯)부터 시작. 담당자 정보(성명·
              연락처·이메일) 3개 필드가 첫 그룹이고, 같은 패턴을 이어서 다른 그룹에도
              적용해 나간다(StepPerformanceInfo.tsx의 CONTACT_GROUP_ID 참고). 라벨
              자체는 이미 위 wizardStrings로 편집 가능하다 — 여기서는 순서·노출만 다룬다. */}
          <Section
            title="위저드 항목 순서 · 노출 — 담당자 정보"
            help="「신청자 정보 및 규모」 화면의 담당자 정보(성명·연락처·이메일) 순서와 노출 여부입니다. 끄면 그 항목이 화면에서 사라지고 필수 입력에서도 빠집니다. 라벨 문구 자체는 위 「위저드 단계 메뉴 이름」 섹션들처럼 화면 문구 키로 편집합니다."
          >
            {(() => {
              const CONTACT_GROUP_ID = "performanceInfo.applicantContact";
              const CONTACT_DEFAULT_ORDER = ["name", "phone", "email"];
              const CONTACT_FIELD_LABELS: Record<string, string> = {
                name: "담당자 (성명)",
                phone: "담당자 연락처",
                email: "담당자 이메일",
              };
              const configured = v.wizardFieldOrders[CONTACT_GROUP_ID];
              const order =
                configured && configured.length > 0
                  ? [
                      ...configured.filter((key) => key in CONTACT_FIELD_LABELS),
                      ...CONTACT_DEFAULT_ORDER.filter((key) => !configured.includes(key)),
                    ]
              ...
              const move = (index: number, direction: -1 | 1) => {
                ...
                const next = [...order];
                [next[index], next[target]] = [next[target], next[index]];
                patch({ wizardFieldOrders: { ...v.wizardFieldOrders, [CONTACT_GROUP_ID]: next } });
              };
              return (
                <ul className="flex flex-col gap-2">
                  {order.map((key, index) => {
                    const fieldId = `${CONTACT_GROUP_ID}.${key}`;
                    const disabled = v.wizardDisabledFields.includes(fieldId);
                    return (
                      <li
                        key={key}
                        className="flex items-center justify-between gap-3 rounded border border-border-soft px-3 py-2"
