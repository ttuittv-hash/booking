/*
  ...
  도로명·지번·**등기된 건물명**만 있고 회사 이름은 없다 — "와이지엔터테인먼트"는 그 건물에
  세든 회사 이름이지 건물명이 아니라서 아무리 쳐도 안 나온다. 가입자는 자기 법인명을
  치는 게 당연해서 이 자리에서 계속 막혔다.

  그래서 법인명 검색은 장소 DB를 따로 쓴다(카카오 로컬 키워드 검색). 다만 이 API 는
...
export async function searchPlaces(query: string): Promise<PlaceSearchResult> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return { status: "ERROR", message: "법인명 검색을 이용할 수 없습니다. 주소로 검색하거나 직접 입력해 주세요." };
    }
    ...
    return { status: "ERROR", message: "법인명 검색이 지연되고 있습니다. 주소로 검색하거나 직접 입력해 주세요." };
  }
}
